  ...
  try {
    ...
    const travauxData = lots.map((lot: any) => {
      ...
      return {
        ...
        cout_materiaux_estime: lot.couts?.materiaux_estime || null,
        cout_mo_pro_estime: lot.couts?.mo_pro_estime || null,
        economie_diy: lot.couts?.economie_diy || null,
...
export async function loadBrouillon(chantierId: string): Promise<LotGenere[]> {
  ...
  return data.map((t) => ({
    ...
    cout_materiaux_estime: t.cout_materiaux_estime || 0,
    cout_mo_pro_estime: t.cout_mo_pro_estime || 0,
    economie_diy: t.economie_diy || 0,
